...
- **PDF 压缩**：清理冗余对象，可选重采样嵌入图片（轻量 / 均衡 / 强压缩三档）
- **PDF 转图片**：按页渲染为 PNG / JPEG，可选 DPI 与页码范围，结果打包 ZIP
...
- **MD 转公众号**：Markdown 一键排版公众号文章，多套主题、代码高亮、图片嵌入，一键复制内联样式 HTML
- **时间戳转换**：Unix 时间戳与日期时间互转，三时区显示，自动识别输入
- **正则测试**：正则表达式匹配/捕获/替换测试，支持常用标志
- **人民币大写**：数字金额转财务规范中文大写，支持角分、千分位
- **二维码生成**：为网址、文本、Wi-Fi、邮件生成自定义二维码
- **ZIP 工具**：多文件打包、ZIP 目录预览、安全解压（防路径穿越与 zip 炸弹）

### 📦 特色功能
- **文件快递**：上传文件生成 6 位取件码，可设有效期与下载次数；支持多文件自动打包、阅后即焚
- **小纸条**：发一段文字生成取件码，对方输入即可查看（可设查看次数、阅后即焚）
