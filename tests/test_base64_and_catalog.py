...
def test_api_tools_catalog():
    ...
    body = r.json()
    assert body["tools"]
    assert any(c["id"] == "pdf" for c in body["categories"])
    assert any(c["id"] == "image" for c in body["categories"])
    assert any(c["id"] == "text" for c in body["categories"])
...
def test_home_lists_categories_and_base64():
    ...
    r = client.get("/")
    assert r.status_code == 200
    assert "PDF 处理" in r.text
    assert "图片处理" in r.text
    assert "文本处理" in r.text
    assert "/#col-pdf" in r.text
    assert "/#col-image" in r.text
    assert "/#col-text" in r.text
...
def test_health_includes_categories():
    ...
    light = client.get("/health?format=json")
    ...
    r = client.get("/health?format=json&detail=1")
    assert r.status_code == 200
    body = r.json()
    assert body["tools"] >= 3
    assert any(c["id"] == "pdf" for c in body["categories"])
