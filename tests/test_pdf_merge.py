...
def test_merge_two_files(tmp_path):
    ...
    stats = merge_invoices(
        str(src1), str(out), pdf2_path=str(src2), add_divider=True
    )
...
def test_page_registered_under_pdf():
...
def test_office_lists_invoice_merge(merge_client):
    client, _ = merge_client
    office = client.get("/")
