...
def test_home_lists_word2pdf():
    ...
    client = TestClient(app)
    # Home exposes category menus (anchor links) and the full tool grid.
    home = client.get("/")
    assert home.status_code == 200
    assert "PDF 处理" in home.text
    assert "/#col-pdf" in home.text
    assert "Word 转 PDF" in home.text
    assert "/tools/word2pdf" in home.text
