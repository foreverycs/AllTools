...
def test_registry_has_text_and_rmb():
...
def test_office_on_home():
    from app import app

    client = TestClient(app)
    home = client.get("/")
    assert home.status_code == 200
    assert "文本处理" in home.text
    assert "/tools/rmb" in home.text
    assert "/tools/pdf-merge" in home.text
    assert "发票合并" in home.text
