...
def parse_proxy_hosts(raw: Optional[str], default=("127.0.0.1", "::1")) -> frozenset[str]:
    """Parse ``TRUSTED_PROXY_HOSTS`` into a set of IPs/hosts.

    Empty config falls back to loopback proxies (the typical single-host
    Baota/Nginx → uvicorn setup). Remote peers can never claim to be these.
    """
    val = (raw or "").strip()
    if not val:
        return frozenset(default)
    parts = [p.strip() for p in val.replace(",", " ").split() if p.strip()]
    return frozenset(parts) if parts else frozenset(default)
...
class Settings:
    ...
    # Public convert/job API rate limit (per client IP).
    # ``api_rate_limit`` = max requests per ``api_rate_window_sec``; 0 disables.
    # ``rate_limit_backend``: memory (process-local) or redis (shared across
    # workers/instances — required for multi-instance deployments).
    api_rate_limit: int
    ...
    # Reverse-proxy peers trusted to rewrite ``request.client`` via
    # X-Forwarded-* (``ProxyHeadersMiddleware``). Rate limiting keys on the
    # (rewritten) client host; only these hosts may supply client identity.
    ...
    # --- SEO ---
    # Public site origin (scheme + host, no trailing slash) used to build
    # absolute URLs for canonical, OpenGraph, sitemap and robots directives.
    # Empty in local/dev deployments disables indexing recommendations.
    site_origin: str
    # Optional override for the default OpenGraph cover image
    # ({site_origin}{root_path}/static/icons/og-cover.png).
    og_image_path: str
    # Global indexability switch. False => robots.txt forbids all and pages
    # emit <meta name="robots" content="noindex,follow">. Useful for staging.
    seo_indexable: bool
...
def _load_settings() -> Settings:
    ...
    return Settings(
        ...
        rate_limit_backend=(os.environ.get("RATE_LIMIT_BACKEND") or "memory")
        .strip()
        .lower()
        or "memory",
        ...
        trusted_proxy_hosts=parse_proxy_hosts(os.environ.get("TRUSTED_PROXY_HOSTS")),
        express_max_bytes=_env_int(
            "EXPRESS_MAX_BYTES",
            # Default: same as general upload limit (typically 50MB).
            max_upload_bytes_val,
            minimum=1024 * 1024,
            maximum=500 * 1024 * 1024,
        ),
        ...
        site_origin=_normalize_site_origin(os.environ.get("SITE_ORIGIN") or ""),
        og_image_path=(os.environ.get("OG_IMAGE_PATH") or "").strip() or "",
        seo_indexable=_env_bool("SEO_INDEXABLE", True),
    )
...
def _normalize_site_origin(raw: str) -> str:
    """Normalize public site origin to scheme://host[:port], no trailing slash.

    Accepts ``https://tools.example.com`` or ``https://tools.example.com/``.
    Rejects anything without a scheme so we never emit malformed canonical
    URLs; returns ``""`` when unset (SEO stays best-effort, fully relative).
    """
    s = (raw or "").strip()
    if not s:
        return ""
    if "://" not in s:
        raise RuntimeError(
            "SITE_ORIGIN must include the scheme, e.g. https://tools.example.com"
        )
    return s.rstrip("/")
